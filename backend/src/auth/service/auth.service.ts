...
export class AuthService {
    ...
    async login(userLogin: UserLoginRequest): Promise<UserLoginResponse>{
        const user = await this.usersService.findOneByEmailWithPassword(userLogin.email.trim().toLowerCase());
        const passwordHash = user ? user.passwordHash : '$2b$12$invalidhashforsecuritypurposesdummy';
    ...
    async resetPassword(token: string, password: string): Promise<UserMessageResponse> {
        ...
        if(!user || !user.resetCodePasswordExpires || user.resetCodePasswordExpires < new Date()) {
